...
#add by dummy, GB to BIG5
from ..langconv import Converter
#end, add by dummy, GB to BIG5
...
def get_title(html, url):
	#add by dummy, GB to BIG5
	dummy_title = re.search('DM5_COMIC_MNAME="([^"]+)', html).group(1)
	dummy_title = Converter('zh-hant').convert(dummy_title)
	return dummy_title
	#end, add by dummy, GB to BIG5
...
def get_episodes(html, url):
	...
	for match in re.finditer('<li>\s*<a href="(/m\d+/)"[^>]*>(.+?)</a>', html, re.DOTALL):
		...
		s.append(Episode(
			#add by dummy, GB to BIG5
			Converter('zh-hant').convert(clean_tags(title)),
			#end, add by dummy, GB to BIG5
			urljoin(url, ep_url)
		))
